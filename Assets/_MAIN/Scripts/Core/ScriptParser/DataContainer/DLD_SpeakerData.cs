...
namespace Core.ScriptParser
{
    ...
    public class DLD_SpeakerData
    {
        ...
        private readonly static Regex parsePattern =
            new Regex(
                $@"{NAME_CAST_ID}|{POSITION_CAST_ID}|{EXPRESSION_CAST_ID.Insert(EXPRESSION_CAST_ID.Length - 1, @"\")}");
        ...
        public DLD_SpeakerData(string rawSpeaker)
        {
            ...
            Debug.Log(
                @$"DLD_SpeakerData Parsed [original={rawSpeaker}][name={name}][castName={castName}][castPosition={castPosition}][castExpressions={string.Join(',', CastExpressions.Select(x => $"{x.layer}:{x.expression}"))}]");
        }

        /// <summary>
        /// For UnitTest
        /// </summary>
        public DLD_SpeakerData(string name, string castName, Vector2 castPosition,
            List<(int layer, string expression)> castExpressions, bool isAppearanceCharacter, bool isCastingPosition)
        {
            this.name = name;
        ...
        public (string name, string castName, Vector2 castPosition, List<(int layer, string expression)> castExpressions
            ) ParseSpeakerData(string rawSpeaker)
        {
            ...
            for (int i = 0; i < matches.Count; i++)
            {
                ...
                if (match.Value == POSITION_CAST_ID)
                {
                    ...
                    string[] axis = castPositionStr.Split(POSITION_AXIS_DELIMITER,
